...
#include "engine/HDWallet.h"

#include "engine/Base58.h"
#include "engine/BinaryCoding.h"
#include "engine/Coin.h"
#include "engine/Mnemonic.h"

#include "engine/TWPublicKeyType.h"
...
boost::optional<PublicKey> HDWallet::getPublicKeyFromExtended(const std::string& extended, TWCoinType coin, const DerivationPath& path)
{
    const auto curve = TW::curve(coin);
...
boost::optional<PrivateKey> HDWallet::getPrivateKeyFromExtended(const std::string& extended, TWCoinType coin, const DerivationPath& path)
{
    const auto curve = TW::curve(coin);
    const auto hasher = TW::base58Hasher(coin);
