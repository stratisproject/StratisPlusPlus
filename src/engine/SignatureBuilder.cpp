...
#include "engine/SignatureBuilder.h"

#include "engine/InputSelector.h"
#include "engine/SigHashType.h"
#include "engine/TransactionInput.h"
...
Result<Transaction, SigningError> SignatureBuilder<Transaction>::sign()
{
    if (plan.error != SigningError("OK")) {
        // plan with error, fail
        return Result<Transaction, SigningError>::failure(plan.error);
    }
    if (transaction.inputs.size() == 0 || plan.utxos.size() == 0) {
        return Result<Transaction, SigningError>::failure("Error_missing_input_utxos");
    }
    ...
    for (size_t i = 0; i < plan.utxos.size(); i++) {
        ...
        if (i < transaction.inputs.size()) {
            ...
            if (!result) {
                return Result<Transaction, SigningError>::failure(result.error());
            }
        }
    }
    ...
    return Result<Transaction, SigningError>::success(std::move(transactionToSign));
}

template <typename Transaction>
Result<void, SigningError> SignatureBuilder<Transaction>::sign(Script script, size_t index,
                                                               const UTXO& utxo)
{
    ...
    if (!result) {
        return Result<void, SigningError>::failure(result.error());
    }
    ...
    if (script.matchPayToWitnessPublicKeyHash(data)) {
        ...
        return Result<void, SigningError>::failure("Error_script_witness_program");
    }
    ...
    return Result<void, SigningError>::success();
}

template <typename Transaction>
Result<std::vector<Data>, SigningError> SignatureBuilder<Transaction>::signStep(
    Script script, size_t index, const UTXO& utxo, uint32_t version) const
{
    ...
    if (script.matchPayToScriptHash(data)) {
        ...
        if (redeemScript.empty()) {
            ...
            return Result<std::vector<Data>, SigningError>::failure("Error_script_redeem");
        }
        return Result<std::vector<Data>, SigningError>::success({redeemScript});
    }
    ...
    if (script.matchMultisig(keys, required)) {
        ...
        return Result<std::vector<Data>, SigningError>::success(std::move(results));
    }
    if (script.matchPayToPublicKey(data)) {
        ...
        return Result<std::vector<Data>, SigningError>::success({signature});
    }
...
Data SignatureBuilder<Transaction>::createSignature(
    ...
    const Script& script,
    const boost::optional<KeyPair>& pair,
    size_t index,
    Amount amount,
...
template <typename Transaction>
boost::optional<KeyPair> SignatureBuilder<Transaction>::keyPairForPubKeyHash(const Data& hash) const
{
