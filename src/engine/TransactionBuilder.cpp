...
#include "engine/TransactionBuilder.h"
#include "engine/Script.h"
#include "engine/TransactionSigner.h"

#include "engine/Coin.h"
#include "engine/SigningError.h"
...
boost::optional<TransactionOutput> TransactionBuilder::prepareOutputWithScript(std::string address, Amount amount, enum TWCoinType coin)
{
...
{
    int count = int(input.outputOpReturn.size() > 0 || !input.outputCustomScript.empty());
    return count;
}
...
TransactionPlan TransactionBuilder::plan(const SigningInput& input)
{
    ...
    if (!input.outputCustomScript.empty()) {
        plan.outputCustomScript = input.outputCustomScript;
    }
    ...
    if (input.amount == 0 && !maxAmount && plan.outputCustomScript.empty() && plan.outputOpReturn.size() == 0) {
        ...
        if (plan.utxos.size() == 0) {
            plan.amount = 0;
            plan.error = SigningError("Error_not_enough_utxos");
        } else {
