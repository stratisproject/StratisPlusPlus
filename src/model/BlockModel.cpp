...
bool BlockModel::fromJson(const web::json::value &val) {
  ...
  if (val.has_field(utility::conversions::to_string_t(U("tx")))) {
    ...
    if (!fieldValue.is_null()) {
      std::vector<std::shared_ptr<AnyType>> refVal_setTx;
...
std::vector<std::shared_ptr<AnyType>> &BlockModel::getTx() { return m_Tx; }

void BlockModel::setTx(const std::vector<std::shared_ptr<AnyType>> &value) {
  m_Tx = value;
  m_TxIsSet = true;
}
