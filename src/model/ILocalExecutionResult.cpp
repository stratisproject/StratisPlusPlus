...
bool ILocalExecutionResult::fromJson(const web::json::value &val) {
  ...
  if (val.has_field(utility::conversions::to_string_t(U("Return")))) {
    ...
    if (!fieldValue.is_null()) {
      std::shared_ptr<AnyType> refVal_setRReturn;
...
std::shared_ptr<AnyType> ILocalExecutionResult::getRReturn() const {
  return m_r_Return;
}

void ILocalExecutionResult::setRReturn(const std::shared_ptr<AnyType> &value) {
  m_r_Return = value;
