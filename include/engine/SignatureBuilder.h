...
private:
    ...
    Data createSignature(const Transaction& transaction, const Script& script, const boost::optional<KeyPair>&,
                         size_t index, Amount amount, uint32_t version) const;

    /// Returns the private key for the given public key hash.
    boost::optional<KeyPair> keyPairForPubKeyHash(const Data& hash) const;
